...
公开 README 中提交并渲染一张不含个人信息的通用流程图，用于解释“指定来源 → 私人
Hub → 全部 Agent → 检查与修复”的运行方式。图中只允许出现通用工具名称、命令和
数据流，不得包含私人 Hub 内容、个人仓库地址、账号、密钥或本机路径。

私人 Hub 保存个人 Skills、共享 MCP 定义、Rules、白名单和退役名称，并使用独立的
私人 GitHub 仓库备份。公开 agent-sync 仓库只保存同步程序、通用示例、测试和说明；
运行时可以读取 Hub 进行同步，但不能把 Hub 内容复制进公开工具仓库。
