...
- [ ] 先写失败测试，覆盖默认只读、drift 返回 2、重名/越界/旧版本拒绝、禁止构造
  非托管命令。
- [ ] 运行 `python3 -m unittest tests/test_sync_multica.py -v`，确认因实现缺失而红。
- [ ] 实现最小配置加载、版本检查、CLI 读取和差异规划。
- [ ] 重跑聚焦测试至绿。
...
- [ ] 先写 `--apply` 失败测试，覆盖 Skill 正文/文件、Agent 完整绑定、Squad 更新与
  应用后复核。
- [ ] 确认测试按预期失败。
- [ ] 实现 Skills → 刷新 → Agent 绑定 → Squads → 最终检查的最小写入流程。
- [ ] 验证重复执行幂等，并确保输出不含 UUID、正文和配置值。
...
- [ ] 先写入口失败测试，证明 `agent-sync multica [--apply]` 正确分发，`all`
  永不调用适配器，未知参数失败。
- [ ] 实现入口并更新 help。
- [ ] 运行新旧单元测试。
...
- [ ] 提供不含私人数据的最小示例。
- [ ] 说明默认只读、退出码、`--apply`、配置边界和不受管理的对象。
- [ ] 将版本升级到 1.7.0。

### 任务 5：真实环境验收

- [ ] 在私人 Hub 建立 `multica/desired-state.json`，执行默认只读检查。
- [ ] 使用临时 Hub 制造 drift，确认返回 2 且远端不变。
- [ ] 对真实期望状态执行一次 `--apply`，再确认只读返回 0。
